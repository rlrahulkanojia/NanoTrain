...
from .backbone import Backbone, InvertedResidual

from .base import BaseNetwork, GraphPath
...
def create_network(num_classes, width_mult=1.0, use_batch_norm=True, onnx_compatible=False, is_test=False):
    ...
    return BaseNetwork(num_classes, base_net, source_layer_indexes,
               extras, classification_headers, regression_headers, is_test=is_test, config=config)


def create_network_predictor(net, candidate_size=200, nms_method=None, sigma=0.5, device=torch.device('cpu')):
